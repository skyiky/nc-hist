...
# Three main patterns:
# Trie/prefix tree --> store words with shared prefixes; reject paths that cannot form a word
# DFS/graph traversal --> Treat cells as vertices and adjacent cells as edges; explore one path deeply before trying another
# Backtracking --> Mark a cell before exploring; then restore it so other paths can use it.
class Solution:
