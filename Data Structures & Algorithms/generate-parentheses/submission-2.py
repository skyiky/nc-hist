class Solution:
    def generateParenthesis(self, n: int) -> List[str]:
        ...
        def backtrack(i):
            if i == n*2: # string contains exactly n pairs
                if not stack: # guarantee every open has been matched
            ...
            if stack: # guarantee every close has an earlier unmatched open
                path.append(')')
                stack.pop()
                backtrack(i + 1)
                path.pop()
